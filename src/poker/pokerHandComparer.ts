  ...
  static compare(leftHand: PokerHand, rightHand: PokerHand) {
    ...
    // Use tiebreaker when dealing with identical hand types
      ...
      for (let i = 0; i < leftHand.tiebreaker.length; i++) {
        ...
        // if everything has same rank, order by card suit other than three of a kind
        if (leftHand.type !== PokerHandType.ThreeOfAKind) {
          type = leftHand.type;
        }
      }
      // if everything has same rank, order by card suit other than three of a kind
      if (leftHand.suit > rightHand.suit && type) {
        console.debug(`[Result] Left hand wins.`);
        return 1;
      } else if (leftHand.suit < rightHand.suit && type) {
        console.debug(`[Result] Right hand wins.`);
        return 2;
      }
